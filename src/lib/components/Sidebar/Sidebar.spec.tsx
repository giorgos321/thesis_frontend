...
describe("Theme", () => {
  ...
  describe("`Sidebar.Collapse`", () => {
    it("should use custom classes", async () => {
      ...
      const labelIcons = collapseLabels().map(
      ...
      labelIcons.forEach((labelicon) =>
        expect(labelicon).toHaveClass("text-gray-400")
      );
      ...
      collapseIcons().forEach((icon) =>
        expect(icon).toHaveClass("bg-gray-200")
      );
    });
  });
  ...
  describe("`Sidebar.Item`", () => {
    it("should use custom classes", () => {
      ...
      const inactiveItems = [
        ...theItems.filter(
          (item) => item !== null && !activeItems.includes(item)
        ),
      ];
      ...
      inactiveIcons.forEach((icon) =>
        expect(icon).not.toHaveClass("bg-gray-300")
      );
      inactiveItems.forEach((item) =>
        expect(item).not.toHaveClass("text-gray-100")
      );
    ...
    it("should use custom classes", () => {
      ...
      render(
        ...
        itemGroups().forEach((group) =>
          expect(group).toHaveClass("text-gray-100")
        );
    });
  });
...
const collapseIcons = () =>
  screen.getAllByTestId("flowbite-sidebar-collapse-icon");

const collapseLabels = () =>
  screen.getAllByTestId("flowbite-sidebar-collapse-label");

const cta = () => screen.getByText("Some content");

const itemContents = () =>
  screen.getAllByTestId("flowbite-sidebar-item-content");
